...
export function registerOfflineObservabilityRoutes(app: Express): void {
  app.post('/api/v1/observability/offline_queue', authMiddleware, async (req: AuthRequest, res: Response) => {
    ...
    const oldestPendingAgeRaw = req.body.oldest_pending_age_ms ?? req.body.oldestPendingAgeMs;
    ...
    if (oldestPendingAgeRaw !== undefined) {
      const parsed = parseNonNegativeInteger(oldestPendingAgeRaw, 'oldest_pending_age_ms');
      if (!parsed.ok) {
        res.status(400).json({ error: parsed.error });
        return;
      }
      oldestPendingAgeMs = parsed.value;
    }
    ...
    try {
      ...
      if (oldestPendingAgeMs !== undefined) {
        observability.recordMetric('offline_queue_oldest_pending_age_ms', oldestPendingAgeMs, tags);
      }
      ...
      observability.info('Offline queue observability reported', {
        ...
        oldest_pending_age_ms: oldestPendingAgeMs ?? 0,
