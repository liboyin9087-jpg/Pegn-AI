...
import { pool, withWorkspaceClient } from '../db/client.js';
...
export function registerKgRoutes(app: Express): void {
  ...
  // 從文字抽取實體與關係並存入 KG
  app.post('/api/v1/kg/extract', authMiddleware, checkPermission('collection:edit'), async (req: AuthRequest, res: Response) => {
  ...
  // 列出 workspace 的所有實體
  app.get('/api/v1/kg/entities', authMiddleware, checkPermission('collection:view'), async (req: AuthRequest, res: Response) => {
    if (!pool) { res.status(503).json({ error: 'DB unavailable' }); return; }
    const workspace_id = req.workspaceId || getWorkspaceIdFromRequest(req);
    ...
    const entities = await withWorkspaceClient(workspace_id, async (client) => {
  ...
  // 取得單一實體的鄰居圖
  app.get('/api/v1/kg/entities/:entity_id/neighbors', authMiddleware, checkPermission('collection:view', 'kg_entity'), async (req: AuthRequest, res: Response) => {
    const { entity_id } = req.params;
    const depth = Math.min(Number(req.query.depth ?? 2), 3);
    const workspaceId = req.workspaceId || getWorkspaceIdFromRequest(req);
    if (!workspaceId) { res.status(400).json({ error: 'workspace_id 為必填' }); return; }

    const graph = await getEntityNeighbors(entity_id, workspaceId, depth);
    res.json(graph);
  });

  // 更新實體內容
  app.patch('/api/v1/kg/entities/:entity_id', authMiddleware, checkPermission('collection:edit', 'kg_entity'), async (req: AuthRequest, res: Response) => {
    if (!pool) { res.status(503).json({ error: 'DB unavailable' }); return; }
    const { entity_id } = req.params;
    const workspaceId = req.workspaceId || getWorkspaceIdFromRequest(req);
    ...
    const result = await withWorkspaceClient(workspaceId, async (client) => {
      return client.query(
        `UPDATE kg_entities
  ...
  app.patch('/api/v1/kg/entities/:entity_id/position', authMiddleware, checkPermission('collection:edit', 'kg_entity'), async (req: AuthRequest, res: Response) => {
    if (!pool) { res.status(503).json({ error: 'DB unavailable' }); return; }
    const { entity_id } = req.params;
    const workspaceId = req.workspaceId || getWorkspaceIdFromRequest(req);
    const { x, y } = req.body ?? {};
  ...
  app.delete('/api/v1/kg/entities/:entity_id', authMiddleware, checkPermission('collection:edit', 'kg_entity'), async (req: AuthRequest, res: Response) => {
    if (!pool) { res.status(503).json({ error: 'DB unavailable' }); return; }
    const { entity_id } = req.params;
    const workspaceId = req.workspaceId || getWorkspaceIdFromRequest(req);
    if (!workspaceId) { res.status(400).json({ error: 'workspace_id 為必填' }); return; }
  ...
  // 列出所有關係
  app.get('/api/v1/kg/relationships', authMiddleware, checkPermission('collection:view'), async (req: AuthRequest, res: Response) => {
    if (!pool) { res.status(503).json({ error: 'DB unavailable' }); return; }
    const workspace_id = req.workspaceId || getWorkspaceIdFromRequest(req);
    if (!workspace_id) { res.status(400).json({ error: 'workspace_id 為必填' }); return; }

    const relationships = await withWorkspaceClient(workspace_id, async (client) => {
