...
export function registerAuthRoutes(app: Express): void {
  ...
  app.post('/api/v1/auth/logout', authMiddleware, (req: AuthRequest, res: Response) => {
    // With JWT, logout is primarily handled by the client discarding the token.
    // In a fully featured environment, you would invalidate it in a Redis blacklist.
    res.json({ success: true, message: 'Logged out successfully' });
  });

  // POST /api/v1/auth/refresh (protected)
  app.post('/api/v1/auth/refresh', authMiddleware, (req: AuthRequest, res: Response) => {
