...
export class SearchService {
  ...
  private vectorSupportCache: { available: boolean; checkedAt: number } | null = null;
  private vectorSupportCacheTtlMs = 5 * 60 * 1000;
  ...
  private async hasVectorSupport(): Promise<boolean> {
    ...
    const now = Date.now();
    if (this.vectorSupportCache && (now - this.vectorSupportCache.checkedAt) < this.vectorSupportCacheTtlMs) {
      return this.vectorSupportCache.available;
    }
    ...
    try {
      ...
      this.vectorSupportCache = { available, checkedAt: now };
      return available;
    } catch (error) {
      console.warn('[search] Failed to detect pgvector extension, fallback to BM25 only', error);
      this.vectorSupportCache = { available: false, checkedAt: now };
      return false;
    }
  }
