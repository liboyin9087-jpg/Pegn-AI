...
class OpenAILLMProvider implements LLMProvider {
  ...
  async generate(prompt: string, input: string): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenAI API request failed (${response.status}): ${errorText.slice(0, 200)}`);
    }
...
export class PromptOpsService {
  ...
  private createLLMProvider(): LLMProvider {
    ...
    if ((preferred === 'openai' || preferred === 'auto') && openaiApiKey) {
      ...
      return new OpenAILLMProvider(openaiApiKey, openaiModelName, openaiBaseUrl);
    }

    if (preferred === 'gemini' && !geminiApiKey) {
