...
[{"name":"OpenAI","entity_type":"org","description":"AI 研究公司"},{"name":"GPT-4","entity_type":"concept","description":"大型語言模型"}]`;
    ...
    const entities = await withWorkspaceClient(workspaceId, async (client) => {
      const rows: KgEntity[] = [];
      for (const e of extracted) {
        ...
        rows.push({ id: entityId, workspace_id: workspaceId, document_id: documentId, ...e });
      }
      return rows;
    });
...
範例：[{"source":"OpenAI","target":"GPT-4","relation_type":"develops","weight":0.9}]`;
    ...
    const relationships = await withWorkspaceClient(workspaceId, async (client) => {
      const rows: KgRelationship[] = [];
...
export async function getEntityNeighbors(
  ...
  try {
    return await withWorkspaceClient(workspaceId, async (client) => {
