...
export class ObservabilityService {
  ...
  private contextStorage = new AsyncLocalStorage<ObservabilityContext>();

  withContext<T>(context: ObservabilityContext, fn: () => T): T {
    return this.contextStorage.run(context, fn);
  }

  private getContext(): ObservabilityContext | undefined {
  ...
  log(level: LogEntry['level'], message: string, context?: Record<string, any>, userId?: string, requestId?: string): void {
    ...
    const resolvedUserId = userId ?? activeContext?.userId;
    const resolvedRequestId = requestId ?? activeContext?.requestId;
...
export function requestTracker(req: Request, res: Response, next: Function) {
  ...
  const runInContext = <T>(fn: () => T): T => observability.withContext({ requestId }, fn);
