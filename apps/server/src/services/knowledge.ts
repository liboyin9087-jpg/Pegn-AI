...
import { graphRAGQuery, type GraphRAGProfile } from './graphrag.js';
...
export interface KnowledgeResult {
  ...
  debug: {
    ...
    graphrag_profile?: GraphRAGProfile;
  };
}
...
export async function knowledgeQuery(params: {
  ...
  if (modeUsed === 'graph') {
    const graph = await graphRAGQuery(query, workspaceId, topK, { includeProfile: true });
    return {
      answer: graph.answer,
      ...
      debug: {
        ...
        graphrag_profile: graph.debug?.profile,
      },
    };
  }
