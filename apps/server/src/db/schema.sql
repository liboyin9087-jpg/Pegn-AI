...
-- Latest Yjs state per document (used by hocuspocus onStoreDocument)
...
CREATE TABLE IF NOT EXISTS agent_step_artifacts (
    id BIGSERIAL PRIMARY KEY,
    run_id UUID NOT NULL REFERENCES agent_runs(id) ON DELETE CASCADE,
    step_id UUID NOT NULL REFERENCES agent_steps(id) ON DELETE CASCADE,
    step_key TEXT NOT NULL,
    artifact_type TEXT NOT NULL CHECK (artifact_type IN ('status', 'input', 'output', 'error')),
...
CREATE INDEX IF NOT EXISTS idx_agent_artifacts_run_time ON agent_step_artifacts(run_id, created_at, id);
CREATE INDEX IF NOT EXISTS idx_agent_artifacts_step_time ON agent_step_artifacts(step_id, created_at, id);
...
-- ============================================================
-- Phase 3: Multi-tenant isolation (RLS foundation)
-- ============================================================
...
DROP POLICY IF EXISTS kg_entities_workspace_isolation ON kg_entities;
CREATE POLICY kg_entities_workspace_isolation
ON kg_entities
FOR ALL
...
DROP POLICY IF EXISTS kg_relationships_workspace_isolation ON kg_relationships;
CREATE POLICY kg_relationships_workspace_isolation
ON kg_relationships
FOR ALL
USING (workspace_id = app_current_workspace_id())
WITH CHECK (workspace_id = app_current_workspace_id());
