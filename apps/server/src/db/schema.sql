...
CREATE TABLE IF NOT EXISTS agent_steps (
    ...
    error TEXT,
    token_usage INTEGER DEFAULT 0,
    -- P2-1: depth inherited from parent run
    depth INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS quota_limits (
    ...
    -- P2-2: optional USD cost ceiling (NULL = unlimited)
    cost_usd_ceiling DECIMAL(10,4) DEFAULT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS usage_records (
    ...
    -- P2-2: estimated USD cost for this period
...
-- Automation definitions (trigger + actions)
CREATE TABLE IF NOT EXISTS automations (
    ...
    -- Conditions (optional, evaluated before actions fire)
    conditions JSONB NOT NULL DEFAULT '[]',
    -- Action list (ordered)
    actions JSONB NOT NULL DEFAULT '[]',
    -- Schedule: cron expression (only used when trigger_type = 'schedule')
    schedule_cron TEXT,
...
CREATE INDEX IF NOT EXISTS idx_automations_enabled ON automations(workspace_id, enabled) WHERE enabled = true;
...
CREATE TABLE IF NOT EXISTS automation_runs (
    ...
    workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
    triggered_by TEXT NOT NULL,   -- 'event' | 'schedule' | 'manual'
...
CREATE TABLE IF NOT EXISTS automation_trigger_events (
    ...
    entity_type TEXT,    -- 'document' | 'collection' | 'comment'
    entity_id UUID,
...
CREATE INDEX IF NOT EXISTS idx_trigger_events_workspace ON automation_trigger_events(workspace_id, event_type, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_trigger_events_unprocessed ON automation_trigger_events(workspace_id, processed) WHERE processed = false;
