...
export async function checkSchema(): Promise<boolean> {
  ...
  try {
    const result = await pool.query(`
      ...
      WHERE table_name IN ('workspaces', 'documents', 'blocks', 'document_snapshots', 'search_index', 'collections', 'collection_views', 'roles', 'quota_limits', 'usage_records', 'audit_logs')
    `);
