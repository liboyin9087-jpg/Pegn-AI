...
import {
  ...
  getOfflineQueueDepth, getOfflineQueueOldestPendingAgeMs, onOfflineQueueChange, replayQueuedMutations,
...
export default function App() {
  ...
  const [aiDefaultTab, setAiDefaultTab] = useState<'chat' | 'search' | 'agent' | 'kg'>('chat');
  ...
  const reportQueueObservability = useCallback(async (payload: {
    ...
    try {
      await reportOfflineQueueMetrics({
        ...
        oldest_pending_age_ms: payload.oldest_pending_age_ms,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const oldestPendingAgeMs = await getOfflineQueueOldestPendingAgeMs();
  ...
  const handleOpenAI = useCallback((prompt?: string) => {
    ...
    setAiDefaultTab(prompt ? 'chat' : 'agent');
    setAiSheetOpen(true);
  }, []);
      ...
      <ErrorBoundary>
        <AiSheet
          ...
          onClose={() => {
            ...
            setAiInitPrompt(undefined);
            setAiDefaultTab('chat');
          }}
          workspaceId={workspace?.id}
          activeDoc={activeDoc}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-1.5">
            ...
            {/* Breadcrumb */}
            <div className="flex items-center gap-1" style={{ fontSize: 13, color: 'var(--color-text-tertiary)' }}>
              <button
              ...
              {activeDoc && (
                <>
                  <span>/</span>
                  <span style={{ color: 'var(--color-text-primary)', fontWeight: 500 }}>{activeDoc.title}</span>
                </>
              )}
              {activeCollection && (
                <>
                  <span>/</span>
                  <span style={{ color: 'var(--color-text-primary)', fontWeight: 500 }}>{activeCollection.name}</span>
                </>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-md transition-colors"
              style={{
            ...
            {!showOnboarding && (
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-panel-hover)'; e.currentTarget.style.color = 'var(--color-text-secondary)'; }}
