import {
  ...
  getOfflineQueueOldestPendingAgeMs as getStoredQueueOldestPendingAgeMs,
...
export async function getOfflineQueueOldestPendingAgeMs(): Promise<number> {
  return getStoredQueueOldestPendingAgeMs();
}
...
export const startBrainstormAgent = (query: string, wsId: string) =>
  api<{ run_id: string }>('/agents/brainstorm', {
    method: 'POST',
    body: JSON.stringify({ query, workspace_id: wsId }),
  });
export const startOutlineAgent = (query: string, wsId: string) =>
  api<{ run_id: string }>('/agents/outline', {
    method: 'POST',
    body: JSON.stringify({ query, workspace_id: wsId }),
  });
...
export interface AgentRunArtifactsResponse {
  ...
  limit: number;
  offset: number;
  timeline: AgentStepArtifact[];
  by_step: Record<string, AgentStepArtifact[]>;
}

export const getAgentRunArtifacts = (runId: string, limit = 200, offset = 0) =>
  api<AgentRunArtifactsResponse>(`/agents/runs/${runId}/artifacts?limit=${limit}&offset=${offset}`);
