...
export function usePresence({ documentId, userId, userName, token }: Options) {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (msg.type === 'presence' && msg.documentId === documentId) {
          ...
          setUsers((msg.users as PresenceUser[])
            .filter(u => u.userId !== userId)
            .map((u) => ({ ...u, lastActiveAt: Date.now() })));
        }
        if (msg.type === 'cursor' && msg.documentId === documentId && msg.userId !== userId) {
          setUsers((prev) => {
            const index = prev.findIndex((item) => item.userId === msg.userId);
  ...
  const sendCursor = useCallback((offset: number) => {
    if (!documentId || typeof offset !== 'number' || offset < 0) return;
    const ws = wsRef.current;
  ...
  return { users, connected, sendCursor };
}
