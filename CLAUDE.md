...
- **Phase**: Phase 2 完成；Phase 3 部分完成（截至 2026-03-02）
...
## Phase 3 進度（已完成 / 部分完成 / 待完成）

### 已完成

- [x] 即時協作（WebSocket / CRDT）
  - [x] 後端已啟動 Hocuspocus sync server（Yjs CRDT）
  - [x] 前端已接入 HocuspocusProvider（即時同步）
  - [x] `/ws` presence/cursor 即時協作通道已上線
- [x] 向量搜尋（pgvector）
  - [x] `CREATE EXTENSION IF NOT EXISTS vector`
  - [x] `vector(768)` 欄位與 ivfflat 索引已建立
  - [x] Hybrid search / GraphRAG 已使用向量查詢
- [x] 容器化部署（Docker Compose 基礎）
  - [x] `docker-compose.yml` 已定義 postgres/redis/api/web
  - [x] `apps/server`、`apps/web` Dockerfile 已落地
  - [x] healthcheck 與 `depends_on` 條件已配置

### 部分完成

- [ ] 多租戶資料隔離（Row-Level Security）
  - [x] `kg_entities` 已啟用 RLS + workspace policy
  - [x] `kg_relationships` 已啟用 RLS + workspace policy
  - [ ] 其餘 workspace-sensitive tables 尚未全面 RLS 化
- [ ] CI/CD Pipeline
  - [x] GitHub Actions CI：typecheck/test/build + docker build
  - [ ] 尚未建立自動 deploy / release / image push pipeline

### 待完成

- [ ] 完整可觀測性（OpenTelemetry）
  - [x] 已有 request tracker、metrics、logs、health
  - [ ] 尚未導入 OpenTelemetry SDK / tracer / span export
